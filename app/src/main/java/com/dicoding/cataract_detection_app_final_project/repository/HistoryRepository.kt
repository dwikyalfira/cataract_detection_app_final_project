        ...
        withContext(Dispatchers.IO) {
            context.dataStore.edit { preferences ->
                ...
                try {
                    ...
                    val recentUserHistory = userHistory.sortedByDescending { it.timestamp }
        ...
        withContext(Dispatchers.IO) {
            context.dataStore.edit { preferences ->
                ...
                try {
                    ...
                    val itemToDelete = allHistory.find { it.id == historyId && it.userId == userId }
                    ...
                    if (itemToDelete != null) {
                        ...
                        // Sync with backend stats
                        try {
                            val apiService = com.dicoding.cataract_detection_app_final_project.data.api.ApiClient.instance
                            val isHealthyStr = if (itemToDelete.isHealthy()) "true" else "false"
                            val response = apiService.deleteHistoryStats(userId, isHealthyStr).execute()
                            if (response.isSuccessful && response.body()?.status == "success") {
                                android.util.Log.d("HistoryRepository", "Stats synced with backend")
                            } else {
                                android.util.Log.e("HistoryRepository", "Failed to sync stats: ${response.message()}")
                            }
                        } catch (e: Exception) {
                            android.util.Log.e("HistoryRepository", "Error syncing stats: ${e.message}")
                        }
                    }
                } catch (e: Exception) {
